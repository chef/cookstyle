...
RSpec.describe RuboCop::CommentConfig do
  ...
  describe '#cop_enabled_at_line?' do
    let(:source) do
      [
        'node.normal[:foo] # rubocop: disable Correctness/Bar',
        'node.normal[:foo] # cookstyle: disable Correctness/Foo',
      ].join("\n")
    end
    ...
    it 'supports disabling cops with the rubocop: disable comment' do
      expect(disabled_lines_of_cop('Correctness/Bar')).to eq([1])
    end

    it 'supports disabling cops with the cookstyle: disable comment' do
      expect(disabled_lines_of_cop('Correctness/Foo')).to eq([2])
    end
  end
end
